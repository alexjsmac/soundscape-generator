...
function getSoundList(dispatch, keyword) {
  return freeSoundService
    ...
    .catch(() => {
      throw new Error(
        `Error getting the list of sounds for keyword: ${keyword}`,
      );
    });
}
...
const freeSoundService = (() => {
  ...
  headers.append(
    "Authorization",
    "Token FTDBgkb5Q3NWqdrtVvNzXNqxIu9TFhj1qrWl2Ue9",
  );
  const get = (url) =>
  ...
  return {
    search: (keyword) =>
      get(`${baseURL}/search/text/?query=${keyword}`).then((res) =>
        res.results.map((result) => {
          return { id: result.id };
        }),
      ),
    sound: (id) =>
      get(`${baseURL}/sounds/${id}/`).then((res) => {
        return {
          name: res.name,
